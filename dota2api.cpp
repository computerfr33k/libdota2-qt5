...
QJsonDocument Dota2API::getMatch(QString match_id)
{
    ...
    QEventLoop event;
    QObject::connect(this->netReply, SIGNAL(finished()), &event, SLOT(quit()));
    event.exec();

    QJsonDocument json = QJsonDocument::fromJson(this->netReply->readAll());
